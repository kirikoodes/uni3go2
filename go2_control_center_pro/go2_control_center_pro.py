...
class App(tk.Tk):
    ...
    def _build_teleop_tab(self):
        ...
        ttk.Label(f, text="Basic movement (Ethernet ready)", font=("Segoe UI", 10, "bold")).grid(row=row, column=0, sticky="w", pady=(8,4)); row += 1
        move = ttk.Frame(f)
        move.grid(row=row, column=0, sticky="w"); row += 1
        ttk.Button(move, text="Forward", command=lambda: self.send_basic_move("forward")).grid(row=0, column=1, padx=4, pady=2)
        ttk.Button(move, text="Left", command=lambda: self.send_basic_move("left")).grid(row=1, column=0, padx=4, pady=2)
        ttk.Button(move, text="Stop", command=lambda: self.send_basic_move("stop")).grid(row=1, column=1, padx=4, pady=2)
        ttk.Button(move, text="Right", command=lambda: self.send_basic_move("right")).grid(row=1, column=2, padx=4, pady=2)
        ttk.Button(move, text="Backward", command=lambda: self.send_basic_move("backward")).grid(row=2, column=1, padx=4, pady=2)
        ttk.Button(move, text="Turn Left", command=lambda: self.send_basic_move("turn_left")).grid(row=3, column=0, padx=4, pady=2)
        ttk.Button(move, text="Turn Right", command=lambda: self.send_basic_move("turn_right")).grid(row=3, column=2, padx=4, pady=2)

        actions = ttk.Frame(f)
        actions.grid(row=row, column=0, sticky="w", pady=(4,6)); row += 1
        ttk.Button(actions, text="Stand Up", command=lambda: self.send_basic_action("StandUp")).pack(side="left")
        ttk.Button(actions, text="Stand Down", command=lambda: self.send_basic_action("StandDown")).pack(side="left", padx=8)
        ttk.Button(actions, text="Balance", command=lambda: self.send_basic_action("BalanceStand")).pack(side="left", padx=8)
    ...
    def send_basic_move(self, cmd):
        if not self.transport:
            messagebox.showwarning("Move", "Connect transport first (Setup tab).")
            return
        ...
        vx, vy, wz = mapping.get(cmd, (0.0, 0.0, 0.0))
        self._log(f"[MOVE] basic command={cmd} -> vx={vx:.2f} vy={vy:.2f} wz={wz:.2f}")
        self._send_move(vx, vy, wz, force=True)
    ...
    def send_basic_action(self, action):
        if not self.transport:
            ...
            return
        try:
            self.transport.send_action(action)
            self._log(f"[ACTION] sent: {action}")
        except Exception as e:
